...
public class ComputeTax {
    ...
    public static void main(String[] args) {
        ...
        // Prompt the user for a filing status
        System.out.println("Enter the filing status: 0 (for Single filer)"
                + "1 (for Married Filing Jointly or Qualified Widower)"
                + "2 (for Married Filing Separately)"
                + "3 (Head of Household)");
        int filingStatus = input.nextInt();

        // Prompt the user for a taxable income
        System.out.println("Enter the taxable income amount: ");
        double income = input.nextDouble();

        // Declare and initialize the tax
        double tax = 0;

        // Compute the tax for status == 0
        if (filingStatus == 0) {
            if (income <= 8350) {
                tax = income * 0.10;
